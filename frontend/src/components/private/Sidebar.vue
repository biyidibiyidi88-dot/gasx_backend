    ...
    <div class="p-4 border-t border-gray-800 bg-gray-900/50 mt-auto" v-if="isOpen || !isMobile">
      <div class="flex items-center space-x-3 mb-4">
        <div class="relative">
          ...
          <div v-if="user.profile_image_url" class="w-10 h-10 rounded-full overflow-hidden border-2 border-blue-500/30">
            <img 
...
const links = {
  ...
  tankMonitoring: [
    ...
    { 
      name: "Analytics", 
      path: "/admin/analytics",  // Changed from "admin/analytics" to "/admin/analytics"
      icon: "M16 8v8m-4-5v5m-4-2v2m-2 4h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" 
    },
    { 
      name: "Tank Locations", 
      path: "/admin/tanks/map",  // Changed from "/tanks/map" to "/admin/tanks/map"
  ...
  alerts: [
    { 
      name: "Active Alerts", 
      path: "/admin/alerts",  // Changed from "/alerts" to "/admin/alerts"
      icon: "M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9", 
      alertCount: 3 
    },
    { 
      name: "Alert History", 
      path: "/admin/alerts/history",  // Changed from "/alerts" to "/admin/alerts/history"
  ...
  configuration: [
    {   
      name: "Profile", 
      path: "/admin/profile",  // Full path with leading slash
      icon: "M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" 
    },
    { 
      name: "System Settings", 
      path: "/admin/settings",  // Changed from "admin/settings" to "/admin/settings"
      icon: "M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z M15 12a3 3 0 11-6 0 3 3 0 016 0z" 
    },
    { 
      name: "User Management", 
      path: "/admin/users",  // Changed from "/users" to "/admin/users"
...
// Improved fetch user profile function
function fetchUserProfile() {
  const profileImage = localStorage.getItem('profile_image');
  if (profileImage) {
    user.value.profile_image_url = profileImage;
...
const handleImageError = () => {
  ...
  user.value.profile_image_url = ''; // This will show the fallback avatar
};
...
const logout = () => {
  try {
    ...
    localStorage.removeItem('profile_image'); // Also clear profile image
  } catch (error) {

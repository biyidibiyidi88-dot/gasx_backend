/*
 * ESP32 Gas Leak Detection System - Persistent Configuration
 ...
 * - No reflashing needed for different users
 */
...
#define SENSOR_ID_ADDR 150
#define API_KEY_ADDR 200
...
const int LED_NORMAL_PIN = 5;      // Green LED for normal status
const int LED_WARNING_PIN = 18;    // Yellow LED for warning
const int LED_CRITICAL_PIN = 19;   // Red LED for critical/leak detection
const int BUZZER_PIN = 21;         // Buzzer for audio alerts
const int CONFIG_BUTTON_PIN = 0;   // Button for configuration mode
...
void setup() {
  Serial.begin(115200);
  Serial.println("=== ESP32 Gas Monitor with Persistent Config ===");
...
void loop() {
  ...
  updateStatusIndicators(severity);
...
// Gas sensor and alert functions (same as before)
int readGasSensor() {

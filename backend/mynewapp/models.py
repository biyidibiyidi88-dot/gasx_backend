...
class CustomUser(AbstractUser, PermissionsMixin):
    ...
    newsletter_subscription = models.BooleanField(_('newsletter subscribed'), default=False)
...
class House(models.Model):
    user = models.ForeignKey(CustomUser, on_delete=models.CASCADE, related_name='houses')
    ...
    def get_sensors_status(self):
        ...
        return self.gas_sensors.aggregate(
            ...
            needs_maintenance=Count('id', filter=models.Q(last_calibration_date__lt=timezone.now()-timezone.timedelta(days=180)))
        )
...
class GasSensor(models.Model):
    ...
    house = models.ForeignKey(House, on_delete=models.CASCADE, related_name='gas_sensors')
    ...
    def needs_calibration(self):
        return self.last_calibration_date < (timezone.now().date() - timezone.timedelta(days=180))
...
class GasReading(models.Model):
    sensor = models.ForeignKey(GasSensor, on_delete=models.CASCADE, related_name='gas_readings')
    ...
    reading_timestamp = models.DateTimeField(_('reading timestamp'), default=timezone.now)
...
class Alert(models.Model):
    ...
    sensor = models.ForeignKey(GasSensor, on_delete=models.CASCADE, related_name='alerts')
    alert_type = models.CharField(_('alert type'), max_length=50, choices=ALERT_TYPES)
    severity_level = models.CharField(_('severity level'), max_length=20, choices=SEVERITY_LEVELS)
...
class Notification(models.Model):
    ...
    notification_method = models.CharField(_('notification method'), max_length=20, choices=NOTIFICATION_METHODS)
    recipient_address = models.CharField(_('recipient address'), max_length=255)
    notification_status = models.CharField(_('status'), max_length=20, choices=NOTIFICATION_STATUSES, default='PENDING')
...
class EmergencyContact(models.Model):
    ...
    relationship = models.CharField(_('relationship'), max_length=20, choices=RELATIONSHIPS)
...
class SensorMaintenanceRecord(models.Model):
    ...
    sensor = models.ForeignKey(GasSensor, on_delete=models.CASCADE, related_name='maintenance_records')
    maintenance_type = models.CharField(_('maintenance type'), max_length=50, choices=MAINTENANCE_TYPES)
    ...
    next_maintenance_due = models.DateField(_('next maintenance due'), blank=True, null=True)
...
class EmergencyAction(models.Model):
    ...
    sensor = models.ForeignKey(GasSensor, on_delete=models.CASCADE, related_name='emergency_actions')
    action_type = models.CharField(_('action type'), max_length=50, choices=ACTION_TYPES)
    action_status = models.CharField(_('status'), max_length=20, choices=ACTION_STATUSES, default='PENDING')
...
class GasRefillRecord(models.Model):
    sensor = models.ForeignKey(GasSensor, on_delete=models.CASCADE, related_name='gas_refills')
    refill_amount = models.DecimalField(_('refill amount'), max_digits=10, decimal_places=2)
